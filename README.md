...
| `readout/freeform_evidence.py` | v2.1 自由曲面局部连续性与概率边界线检查 |
...
| `readout/RESULTS_V21.md` | 叶片 1～4 与既有模型的 v2.1 验证 |
...
当前默认是 v2.1 曲面连续性读出：连通微区域 → 几何/概率组装与同一曲面、自由曲面局部验证合并 → 使用相同连续性判据的重新切分 → 最小规模约束 → 边界细化和合并复核。没有旧算法模式开关。
...
v2.1 为自由曲面增加局部证据：不强求整片能用一个解析方程拟合，结合接口概率、相对两侧的概率凸起、局部法向和曲率判断。叶片上的剩余过分割尚未全部解决，见本轮验证记录。
...
# 修改算法并运行测试
git add readout/geometry_readout.py readout/surface_evidence.py readout/freeform_evidence.py readout/test_readout.py
git diff --cached
