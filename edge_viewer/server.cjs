...
const types = {'.wasm':'application/wasm','.html':'text/html; charset=utf-8','.js':'text/javascript','.mjs':'text/javascript','.css':'text/css','.csv':'text/csv','.json':'application/json'};
http.createServer((req,res)=>{
  const u = new URL(req.url,'http://localhost');
  if(u.pathname==='/api/models') {
    const models=(fs.existsSync(data)?fs.readdirSync(data):[]).filter(n=>n.endsWith('__edge_boundary_probability.csv')).map(n=>{
